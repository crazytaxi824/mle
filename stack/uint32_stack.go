...
func NewUint32Stack(opt *Option) *uint32Stack {
	if opt == nil {
		opt = &Option{
			AllowDuplicate: true,
	...
	return &uint32Stack{allowDupl: opt.AllowDuplicate, stackType: opt.StackType}
}

// 单调栈，会踢出大于自己的元素
// n 表示有多少元素被踢出
// return n means how many items has been removed from stack
func (s *uint32Stack) Push(i uint32) (n int, res []uint32) {
	switch s.stackType {
	case ASCStack:
...
func (s *uint32Stack) pushToASCStack(i uint32) (n int, res []uint32) {
	index := -1
...
func (s *uint32Stack) pushToDESCStack(i uint32) (n int, res []uint32) {
	index := -1
...
func (s *uint32Stack) pushToNormalStack(i uint32) (n int, res []uint32) {
	if !s.allowDupl {
		for k := range s.items {
			if s.items[k] == i {
				return 1, []uint32{i}
...
func (s *uint32Stack) elements() []uint32 {
	return s.items
}
...
// 重置 stack
func (s *uint32Stack) Reset() {
	s.items = make([]uint32, 0)
}
...
// 位置 1 代表栈顶，即最后一个元素。
// 位置 -1 代表没有找到该元素。
func (s *uint32Stack) Search(n uint32) int {
	switch {
...
// 二分法查找
func (s *uint32Stack) searchASCDichotomy(n uint32) int {
	lenS := len(s.items)
...
func (s *uint32Stack) searchDESCDichotomy(n uint32) int {
	lenS := len(s.items)
...
func (s *uint32Stack) searchNormalStack(n uint32) int {
	lenS := len(s.items)
	for i := lenS - 1; i >= 0; i-- {
		if s.items[i] == n {
			return lenS - i
...
func (s *uint32Stack) Range(fn func(element uint32) bool) {
	for k := range s.items {
		if !fn(s.items[k]) {
			return
		}
...
func (s *uint32Stack) Index(index int) (uint32, error) {
