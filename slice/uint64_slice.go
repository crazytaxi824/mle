...
func (uint64Type) IndexOf(s []uint64, n uint64) int {
	for k := range s {
		if s[k] == n {
			return k
		}
...
func (uint64Type) LastIndexOf(s []uint64, n uint64) int {
	if len(s) < 1 {
		return -1
	}
...
func (uint64Type) SameElements(a, b []uint64) bool {
	if len(a) != len(b) {
		return false
	}

	tmp := make(map[uint64]int)
	for _, v := range a {
		tmp[v]++
	}
...
func (uint64Type) Contains(a, b []uint64) bool {
...
func (uint64Type) ContainsAny(a, b []uint64) bool {
	tmp := make(map[uint64]struct{})
	for _, v := range a {
		tmp[v] = struct{}{}
	}
...
func (uint64Type) Intersection(a, b []uint64) []uint64 {
	tmp := make(map[uint64]struct{})
	for _, v := range a {
		tmp[v] = struct{}{}
	}
...
func (uint64Type) Union(a, b []uint64) []uint64 {
	tmp := make(map[uint64]struct{})
	for _, v := range a {
		tmp[v] = struct{}{}
	}
...
func (uint64Type) Difference(a, diff []uint64) []uint64 {
	tmp := make(map[uint64]struct{})
	for _, v := range a {
		tmp[v] = struct{}{}
	}
...
func (uint64Type) SymmetricDifference(a, b []uint64) []uint64 {
	tmp := make(map[uint64]struct{})
	for _, v := range a {
		tmp[v] = struct{}{}
	}
