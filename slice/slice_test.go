...
var myNum = []int{1, 2, 3, 4, 5, 4, 3, 2, 1}

func TestIndexOfInt(t *testing.T) {
	ast := assert.New(t)
	ast.Equal(Int().IndexOf(myNum, 2), 1)
	ast.Equal(Int().LastIndexOf(myNum, 2), 7)

	ast.Equal(Int().Equal(myNum, []int{1, 2, 3, 4, 5, 4, 3, 2, 1}), true)
	ast.Equal(Int().Equal(myNum, []int{1, 2, 3, 4, 5, 4}), false)
